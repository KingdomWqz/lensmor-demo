...
- 通用 Bug 判定规则
  - 如果核心能力已经被产品主界面、真实交互或 API 强证据证明，而问题只出现在次级页面、日志页、状态同步、辅助展示或其他 supporting surface，不要因为这个 Bug 直接把需求降为 `partial` 或 `missing`
  - 这种情况下，优先保持状态为 `implemented`，并在该维度上适度扣分，通常扣 `0.05-0.2`
  - 只有当 Bug 破坏了需求本身的核心闭环、导致用户无法有效使用，或使关键结果仍未被验证时，才应降为 `partial`
...
- `UR-3 Continuous collection and interpretation`
  ...
  - 如果持续采集和解释能力已经被主工作台、列表 API、详情 API 或真实时间戳证明，而日志页/运行历史页为空或显示异常，通常应记为已实现但适度扣分，而不是直接降为 `partial`
