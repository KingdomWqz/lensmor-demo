...
5. 重新检查网络请求和时间戳，确认创建后的状态确实发生了变化。
...
3. 重新检查 dashboard、inbox、目标详情页和相关 API，确认是否出现了新的时间戳或新条目。
4. 如果被监控夹具是指定的 `picpi` demo 目标且仍无变化，则触发其上游 timer API，安全地制造 demo 内容漂移。
5. 等待后重新运行监控，再次检查产品各界面。

对于 `picpi` 夹具，使用下面的精确序列：

```bash
opencli browser audit tab create https://picpi-iota.vercel.app/
curl --request POST \
...
- 触发 timer 后，回到被监控产品，先确认新变化项确实出现，再提高任何相关得分。
...
- `UR-1 Quick setup`
  - Full：用户可以通过清晰流程新增或配置监控目标，且创建后的目标能在一个或多个产品界面中看到
  - Partial：存在配置 UI，但目标定义流程不完整、被阻塞，或未证明会持久化
